...
#
# php -S localhost:xxxx -t public/ tests/router.php
#
...
if (preg_match('/\/([_0-9a-zA-Z-]+\/)?(wp-.*)/', $request_path, $matches)) {
    ...
    // The file is a user upload which is not present in the local file system
    } elseif (strpos($request_path, 'wp-content/uploads') !== false) {

        header('Location: '.$production_baseurl.$request_path, true, 302);
        return;
